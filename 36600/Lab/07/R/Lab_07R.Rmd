...
M <- cor(df)
corrplot(M, method = "ellipse", tl.pos="n")
```
...
cums <- data.frame(cumsum(pve), c(1:length(cumsum(pve))))
names(cums) <- c("cum", "num")
ggplot(cums, aes(x=num,y=cum)) + geom_line() + geom_point(size=2)
```
```
Around 5
```
...
```{r}
ggplot(data=df,mapping=aes(x=pr.out$x[,1],y=pr.out$x[,2], color=response)) +
geom_point(size=0.5)
```
...
```{r}
# PC2 and PC3
ggplot(data=df,mapping=aes(x=pr.out$x[,2],y=pr.out$x[,3], color=response)) +
geom_point(size=0.5)

# PC3 and PC4
ggplot(data=df,mapping=aes(x=pr.out$x[,3],y=pr.out$x[,4], color=response)) +
geom_point(size=0.5)
```
...
```{r}
ggplot(data=df,mapping=aes(y=pr.out$x[,1],x=response)) +
