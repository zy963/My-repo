...
\newcommand{\dueDate}{Wednesday, January 26th}
...
\item The \LaTeX template for this assignment is available \href{https://www.overleaf.com/read/ktntrmfmddyb}{here}.
...
\begin{parts}
    ...
    \part[1] \sone How many grace days do you have in total for all homework? Can you combine grace days with late days to extend a homework submission deadline by more than 4 days?
...
\begin{parts}
    ...
    \part[1] \sone What is the probability of observing any combination of this data (4 heads and 1 tails), assuming it was generated by flipping a coin X with an unequal probability of heads (1) and tails (0), where the distribution is $P(X = 1) = 0.75$, $P(X = 0) = 0.25$?
...
\clearpage
    ...
    \part[2] What is $P(B=b_1 | A = a_2, C = c_4)$? If this value cannot be computed, write N/A.
    
    \begin{your_solution}[height=2cm, width=4cm]
    \end{your_solution}
    
    \part[2] What is $P(B=b_2 | A = a_3, C = c_3)$? If this value cannot be computed, write N/A.
...
\begin{parts}
    ...
    \part[2] \sone Suppose that $f(\xv|\thetav)=\xv^T\thetav$, where $\xv, \thetav \in \mathbb{R}^n$. The function $g(\thetav)$ is defined as $g(\thetav) = (f(\xv^{(1)}|\thetav) - y^{(1)})^2$ for $\xv^{(1)} \in \mathbb{R}^n$ and $y^{(1)} \in \mathbb{R}$. What is the function type of $g(\thetav)$:
...
\begin{parts}
    ...
    \part[1] \sone Consider the matrix $\mathbf{X}$ and the vectors $\mathbf{y}$ and $\mathbf{z}$ below: \textbf{X}=$\begin{bmatrix} 1 & 4 \\ 2 & 6 \end{bmatrix}$, \textbf{y}=$\begin{bmatrix} 2 \\ 1 \end{bmatrix}$, \textbf{z}=$\begin{bmatrix} 4 \\ 3 \end{bmatrix}$. What is the inner product of the vectors \textbf{y} and \textbf{z}? (this is also sometimes called the dot product)
    ...
    \part[1] \sone Using the same values for \textbf{X}, \textbf{y}, and \textbf{z} as above, what is the product of \textbf{Xy}?
...
\clearpage
    ...
    \part[2] \sone Preparing for his linear algebra final, Joe is finding eigenvectors and eigenvalues for different matrices. Joe finds out that some matrix A (not given) has eigenvalues 4 and 5. To find the associated eigenvectors for eigenvalue 4, he then solves the equation $\mathbf{A} \vv = 4 \vv$ and finds two solutions, \\
    $\begin{bmatrix} 3 \\ 117 \\ 9 \end{bmatrix}$ and $\begin{bmatrix} 1 \\ 39 \\ 3 \end{bmatrix}$. He concludes that these vectors are the eigenvectors corresponding to the eigenvalue of 4. Which statement regarding his solution is true?
    ...
    \begin{checkboxes}
        ...
        \choice The solution is correct because there may be multiple eigenvectors corresponding to an eigenvalue, and eigenvectors of a matrix are linearly dependent.
        \choice The solution is correct because both vectors are solutions to $\mathbf{A} \vv = 4 \vv$. 
    \end{checkboxes}
...
\begin{parts}
    \part[1] \sone If $f(n)=\ln(n)$ and $g(n)=\log_3(n)$ which of the following are true?
    ...
    \part[1] \sone If $f(n)=n^{10}$ and $g(n)=10^n$ which of the following are true?
    ...
    \part[2] \sone Using the tree shown in Figure \ref{fig:family}, how many nodes would depth-first-search visit in finding Mia Tindall (including her node)? Assuming we search left-to-right and top-down.
    ...
    \begin{your_solution}[height=1.5cm, width=\textwidth/2]
    %solution
    \end{your_solution}
